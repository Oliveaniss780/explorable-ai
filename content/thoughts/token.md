...
A **token** is the atomic unit of text a language model reads and writes — usually a common word or a fragment of one (roughly 3–4 characters of English on average). Text is split into tokens by a _tokenizer_ before the model ever touches it, and the model generates one token at a time.
